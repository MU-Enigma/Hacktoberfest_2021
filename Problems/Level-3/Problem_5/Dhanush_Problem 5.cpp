...
int recur(vector<int> ar,int x1,int x2)
{
    ...
    {
        cout<<ar[x1]<<" "<<ar[x1+1]<<"\n";
        return ar[x1]<ar[x1+1]?x1:x1+1;
    }
    int sum1=0,sum2=0;
    for(int i=x1;i<x1+x2/3;i++)
    {
        sum1+=ar[i];
    }
    for(int i=x1+x2/3;i<x1+x2/3+x2/3;i++)
    {
        sum2+=ar[i];
    }
    ...
    if(sum1==sum2)
    {
        return recur(ar,x1+2*(x2/3),x2-2*(x2/3));
    }
    if(sum1<sum2)
    {
        return recur(ar,x1,x2/3);
    }
    if(sum2<sum1)
    {
        return recur(ar,x1+x2/3,x2/3);
    }
}
